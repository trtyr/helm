...
use crate::grpc::connection_registry::{ConnectionRegistry, RegisteredConnection};
...
use crate::store::Db;
use helm_proto::pb::{AgentMessage, Register};
...
impl ForwardManager {
    ...
    pub async fn remove(&self, host_id: &Uuid) {
        ...
        if let Some(tx) = tx {
            let _ = tx.send(()).await; // 停止信号：拨号循环可能已自行退出，失败无副作用
...
use connect::connect_once;

/// 阶段 7：装配入站上下文并消费入站流，退出时统一记账（`on_disconnect`）。
pub(super) async fn pump_forward_inbound(
    deps: &ForwardDeps,
    host_id: Uuid,
    agent_id: &str,
    register: Register,
    ...
    let (disc_reason, disc_detail) =
        forward_inbound_loop(&mut ctx, agent_id, &mut inbound, &mut kick_rx, stop_rx).await;
    ctx.on_disconnect(&disc_reason, &disc_detail).await;
    anyhow::bail!("stream closed")
}

/// 阶段 7a：三路 select 主循环（停止信号 / 被顶号 / 流消息），返回断开原因供记账。
async fn forward_inbound_loop(
    ctx: &mut InboundCtx,
    agent_id: &str,
    inbound: &mut Streaming<AgentMessage>,
    kick_rx: &mut watch::Receiver<bool>,
    stop_rx: &mut mpsc::Receiver<()>,
) -> (String, String) {
    ...
    loop {
        tokio::select! {
            ...
            _ = async {
                // watch 通道：发送端 drop 时 wait_for 返回 Err，语义等价于「不会再有 kick」
    ...
    (disc_reason.to_string(), disc_detail)
}
