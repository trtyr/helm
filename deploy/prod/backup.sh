...
if [[ "$ACTION" == "backup" ]]; then
	...
	docker compose exec -T postgres pg_dump -U "$USER_DB" -d "$NAME_DB" | gzip >"$OUT"
	echo "✓ 完成：$(du -h "$OUT" | cut -f1)"

	# 轮转：只保留最近 KEEP 份
	mapfile -t OLD < <(ls -1t "$OUT_DIR"/helm-*.sql.gz 2>/dev/null | tail -n +$((KEEP + 1)))
	if ((${#OLD[@]})); then
		printf '%s\n' "${OLD[@]}" | xargs rm -f
		echo "✓ 清理旧备份 ${#OLD[@]} 份（保留最近 $KEEP）"
	fi
else
	[[ -f "$RESTORE_FILE" ]] || {
