...
use super::util::{Entry, Scanner, clsid_server, extract_exe, hkey_local, reg_get_value, reg_subkeys, reg_values};
...
fn com_addins(sc: &mut Scanner) {
    for hive_label in ["HKLM", "HKCU"] {
        let root_hive = if hive_label == "HKLM" { hkey_local() } else { HKEY_CURRENT_USER };
        ...
        for ver in reg_subkeys(root_hive, office_root) {
            ...
            for app in OFFICE_APPS {
                ...
                for id in reg_subkeys(root_hive, &addins_root) {
                    ...
                    let get =
                        |k: &str| reg_get_value(root_hive, &full, k).filter(|s| !s.trim().is_empty());
