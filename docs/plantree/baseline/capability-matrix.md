...
运维平台的能力现状评估（**参考 C2 控制面能力，不做隐蔽性 / 规避设计**）。对照「连接模式 / 监听器 / 在线离线 / 控制 / 监控 / 持久化 / 安全 / API 完整性」八个能力域，标注已实现与缺失。每条引用真实文件或 API 佐证，不虚构能力。以当前 `HEAD`（Phase 8 已落地，`a3e0300`）为基线。
...
| 监控 | ✅ 齐 | cpu/mem/disk/net/proc + 阈值告警 + 时序保留 |
...
- 阈值告警：`AlertService::threshold_for`（cpu/mem/disk > 90%）→ 落 `alerts` 表 → `GET /api/v1/alerts`。
