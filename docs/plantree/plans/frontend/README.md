...
| **M3 监控与任务**（服务/进程/网络/指标/任务 + uPlot） | ✅ 已落地 | helm-console `6f22974`：verify-m3.mjs 17 项真 agent 联调全过（服务启停 + 日志快照/WS tail、进程 kill toast、网卡卡、指标历史 1h + 实时 WS 推点、任务创建直达详情 + 输出回放、终端 F34 主题/全屏）；uPlot 按决策 006 集成；全局 toast + useWsStream/useBinaryStream（修 StreamRegistry binary 帧解码）；vitest 35 测试 |
